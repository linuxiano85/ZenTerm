...
impl GpuMock {
        ...
        match limit {
            25 | 50 | 75 | 100 => {
                ...
                if self.current_usage > limit as f32 {
                    ...
                    debug!("Adjusted GPU usage to {:.1}% due to new limit", self.current_usage);
                }
                
                Ok(())
            }
            _ => Err(format!("Invalid GPU limit: {}. Must be 25, 50, 75, or 100", limit)),
        }
    }
    ...
    pub fn apply_limit(&self) -> Result<(), String> {
        info!("Applying GPU limit: {}% (current usage: {:.1}%)", 
