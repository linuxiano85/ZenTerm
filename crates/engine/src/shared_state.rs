...
use crate::{Config, EventBus, CommandRegistry, GpuMock, Theme, VoiceMock, Wizard};
...
impl SharedAppState {
    ...
    pub fn new() -> Self {
        ...
        gpu_mock.set_limit(config.gpu.limit_percentage).unwrap_or_else(|e| {
        ...
        match event {
            AppEvent::GpuLimitChanged(limit) => {
                if let Err(e) = guard.gpu_mock.set_limit(limit) {
                    ...
                    self.add_log_message_internal(&mut guard, format!("GPU Error: {}", e), LogLevel::Error);
                } else {
                    guard.config.gpu.limit_percentage = limit;
                    guard.config.mark_dirty();
                    self.add_log_message_internal(&mut guard, format!("GPU limit set to {}%", limit), LogLevel::Info);
                }
            }
            AppEvent::ThemeToggled(dark_mode) => {
                ...
                self.add_log_message_internal(&mut guard, format!("Theme changed to {}", theme_name), LogLevel::Info);
            }
            AppEvent::VoiceToggled(enabled) => {
                if enabled && guard.voice_mock.is_none() {
                    ...
                    self.add_log_message_internal(&mut guard, "Voice engine started".to_string(), LogLevel::Info);
                } else if let Some(ref voice) = guard.voice_mock {
                    voice.set_enabled(enabled);
                    let status = if enabled { "enabled" } else { "disabled" };
                    self.add_log_message_internal(&mut guard, format!("Voice engine {}", status), LogLevel::Info);
                }
            ...
            AppEvent::WizardOpened => {
                guard.wizard.open();
                self.add_log_message_internal(&mut guard, "Setup wizard opened".to_string(), LogLevel::Info);
            }
            AppEvent::WizardClosed => {
                guard.wizard.close();
                self.add_log_message_internal(&mut guard, "Setup wizard closed".to_string(), LogLevel::Info);
            }
            AppEvent::ConfigSaveRequested => {
                if let Err(e) = guard.config.save() {
                    error!("Failed to save config: {}", e);
                    self.add_log_message_internal(&mut guard, format!("Config save failed: {}", e), LogLevel::Error);
                } else {
                    guard.config.dirty = false;
                    self.add_log_message_internal(&mut guard, "Configuration saved".to_string(), LogLevel::Info);
                }
            }
            ...
            AppEvent::QuitRequested => {
                ...
                self.add_log_message_internal(&mut guard, "Quit requested".to_string(), LogLevel::Info);
            }
        }
    }
    ...
    fn add_log_message_internal(&self, guard: &mut AppStateInner, message: String, level: LogLevel) {
        let entry = LogEntry {
...
mod tests {
    ...
    fn test_log_messages() {
        ...
        let has_test1 = messages.iter().any(|m| m.message.contains("Test message 1"));
        let has_test2 = messages.iter().any(|m| m.message.contains("Test message 2"));
        assert!(has_test1);
        assert!(has_test2);
    }
